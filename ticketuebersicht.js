$(document).ready(function () {
    ...
    function initDB() {
        ...
        request.onupgradeneeded = function(event) {
            ...
            if (!db.objectStoreNames.contains(storeName)) {
                let store = db.createObjectStore(storeName, { keyPath: ["SID", "AUFTNR"] });
    ...
    function fetchData() {
        const API_ENDPOINT = 'https://otc.bsv.net/api/mgwebrequester.dll?appname=FlowRequester&PRGNAME=requesterPortale&ARGUMENTS=-Agetposerviceauftrag';
        ...
        if ($("#grid").data("kendoGrid")) {
                ...
                schema: {
                    model: {
                        fields: {
                            AUFTNR: { type: "string" },
                            Modell: { type: "string" },
                            Klartext_char: { type: "string" },
                            Kunde: { type: "string" },
                            EingangDatum: { type: "string" },
                            EingangZeit: { type: "string" },
                            TerminDatum: { type: "string" },
                            TerminZeit: { type: "string" },
                            Status: { type: "string" },
                            Event: { type: "string" },
            ...
            $("#grid").kendoGrid({
                ...
                columns: [
                    ...
                    { field: "Anschrift", title: "Anschrift", width: "120px" },
                    { field: "EingangDatum", title: "Eingangsdatum", format: "{0:dd.MM.yy}" },
                    { field: "EingangZeit", title: "Eingangszeit" },
                    { field: "Aktion", title: "Aktion", width: "120px" },
                    { field: "Servicestatus", title: "Servicestatus", width: "120px" },
                    { field: "Information", title: "Information", width: "120px"}
                ]
            });
        }
    }
    ...
    $('#searchBox').on('input', function() {
        ...
        grid.dataSource.filter({
            logic: "or",
            filters: [
                { field: "AUFTNR", operator: "contains", value: value },
                ...
                { field: "EingangDatum", operator: "contains", value: value },
                { field: "EingangZeit", operator: "contains", value: value },
                { field: "Anschrift", operator: "contains", value: value },
                { field: "Aktion", operator: "contains", value: value },
                { field: "Servicestatus", operator: "contains", value: value },
                { field: "Information", operator: "contains", value: value }
            ]
        });
    });
